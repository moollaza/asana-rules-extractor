 ...
 * together ("Areais set to"). Skips facepiles (avatars plus a bare
...
/**
 * The pill in a card ("Video Player") is the referenced object, so read it off
 * the element rather than regexing the concatenated text. Field-name pills
 * ("Area") are excluded: the testid sits on a span inside the pill.
 */
