...
export const instructionsEditor = `<div class="AutomationCanvasAiGuidanceFloatingCard"><div class="AutomationCanvasAiGuidanceFloatingCard-textEditor"><div class="ProseMirror" contenteditable="true">
  <p>When Surface Area is "Static":</p>
  <p><br></p>
  <p>Determine if </p>
...
export const cards = {
  ...
  fieldChanged: card(null, `${fieldPill("Surface Area")} is changed`),
  enumCondition: card(
    "Check if",
    `${fieldPill("Surface Area")} is set to <span class="AutomationConfigurationContentPills">${pill("Search Ads")}</span>`,
  ),
  truncated: card(
    "Otherwise if",
    `${fieldPill("Surface Area")} is one of <span class="AutomationConfigurationContentPills">${pill("Alpha")}<span>+25</span></span>`,
  ),
  moveToSection: card(
    "Do this",
