...
describe("shapeRule", () => {
  it("hoists the joiner into one all/any group and drops kind", () => {
    const shaped = shapeRule(
      raw({
        when: [entry("Task is added to this project"), entry("OR Area is changed")],
        branches: [
          {
            kind: "Check if",
    ...
    expect(shaped.when).toEqual({
      any: ["Task is added to this project", "Area is changed"],
    });
  ...
  it("splices expanded values into a truncated condition and flags unexpanded ones", () => {
    const full = shapeRule(
      raw({
        branches: [
          {
            kind: null,
            if: [entry("Area is one of A, +25", { values: ["A", "B", "C"] })],
            do: [entry("Move task to section S", { pills: ["S"] })],
          },
        ],
      }),
    );
    expect(full.branches[0].if).toEqual({ all: ["Area is one of A, B, C"] });
    ...
    const partial = shapeRule(
      raw({
        branches: [
          {
            kind: null,
            if: [entry("Area is one of A", { incomplete: true, hiddenValueCount: 25 })],
            do: [],
          },
        ],
      }),
    ...
    expect(partial.branches[0].if).toEqual({
      all: [{ condition: "Area is one of A", incomplete: true, hiddenValueCount: 25 }],
    });
  });
});
